...
def get_image(sheet,pokemon_type,width,height,scalex,scaley): # this function called in pokemoninparty page and maybe in menu draw
...
pysical_tm_img , special_tm_img , status_tm_img , energy_tm_img , reg_tm_img ,range_tm_img, trash_del_tm_img , trainer_box_img = tm_images_lst[0], tm_images_lst[1],tm_images_lst[2]\
...
def draw_multyline_text(surface,text,word_pos,font,color,max_width,max_height,rect_pos = None,button_rect_color = None , text2 = None):
...
##    x_fixed , y_fixed = s    
    ...
    if rect_pos is not None:
        ...
        rect_color_big = pygame.Rect(word_pos[0]-20-fixed_x,word_pos[1]-20-fixed_y,max_width+20,b+20-word_pos[1])
        ...
        if button_rect_color is not None:
            ...
            left_rect_color_small = pygame.Rect(rect_color_big.x +10 +fixed_x ,fixed_y+rect_color_big.bottom - 35,50,28)
            right_rect_color_small = pygame.Rect(rect_color_big.right - 60 + fixed_x,fixed_y+rect_color_big.bottom - 35,50,28)
...
def draw_bg():
    ...
    for i in range(len(snowFall)):
        if i % 2 == 0:
            ...
            size =3
        pygame.draw.circle(screen,(random.randint(235,255),random.randint(235,255),random.randint(235,255)),snowFall[i],size)
...
def draw_menue():
    ...
    menue_rect = pygame.Rect(x_cord//2 + screen_scroll-(width_rect//2),y_cord-height_rect-25,width_rect,height_rect)
    ...
    if pos[1] > menue_rect.y - 10 or draw_menu_keyboard:
        ...
        if (pokeball_btn.draw(screen) or pokeball_throw_keyboard) and player.pokeball > 0 and player.pokeball_thrown == False:

            pokeball = POKEBALL(player.rect.centerx-int(player.direction*player.rect.size[0]*0.5),player.rect.top,player.direction)
...
class Player(pygame.sprite.Sprite): 
    def __init__(self,char_type,char, x, y,hp,scale, speed,fake_speed,ammo,grenades,typ1,typ2,energy,atk,deff,spa,spd,tm_lvl_dict):#,x_states,y_states):
        ...
        for animation in animation_types:
            ...
            for i in range(len(num_of_frames)):
                img = pygame.image.load('images\\pokemon_images\\' + self.char_type + f'\\{animation}\\{i}.png').convert_alpha() # added from part4 only "convert_alpha()"
                #img = pygame.transform.scale(img,(int(img.get_width()*0.2),int(img.get_height()*0.2))) # image according screen scale
                img = pygame.transform.scale(img,(int(TILE_SIZE*scale),int(TILE_SIZE*scale))) # image according screen scale
...
##            self.animation_list_states.append(img_for_states)
        ...
        self.img_for_states = pygame.image.load('images\\pokemon_images\\' + self.char_type + '\\idle\\0.png').convert_alpha() #main img
    ...
    def move(self,moving_left, moving_right):
        ...
        dy = self.vel_y
        self.altitude += 1
        if self.altitude >  altitude and not pygame.sprite.spritecollide(self,water_group,False) and self.char == 'player' and\
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            bullet = Bullet(self.rect.centerx  + (1.5*self.rect.size[0] * -self.direction), self.rect.centery+10,self.direction,(self.img.get_width(),self.img.get_height()))
    ...
    def PokemonInParty(self,x,y,lvl_100_states):
        ...
        if self.state_blit:
            ...
            if methode_lst[self.index_methode] == 2:
                ...
                state_lst_text = ['HP :','Attack :','Deffence :','Sp.Atk :','Sp.Def :','Energy :','Reg-Eng :','Speed :']
                state_lst = [self.max_health,self.atk,self.deff,self.spa,self.spd,self.energy,self.reg,self.fake_speed]
                ...
                for i,state in enumerate(state_lst_text):
                    ...
                    screen.blit(state_text,(880+(font2_n.render('Reg-Eng :',True,BLACK).get_width())-state_text.get_width(),35 + (i*state_text.get_height())))
                for i, state in enumerate(lst_to_compare):
                    state_text = font2_n.render(str(int(state)),True,BLACK)
                    screen.blit(state_text,(880+(font2_n.render('Reg-Eng :',True,BLACK).get_width()) + 5,35 + (i*state_text.get_height())))
                    state_bar_lengh = 200
                    ...
                    pygame.draw.rect(screen,rect_state_color,(1005,42 + (i*state_text.get_height()),int(ratio_state*state_bar_lengh),15))
                ...
                for i in range(len(self.TM_LST)):
                    TM_RECT_grid = pygame.Rect(width - rect_width_TM ,67+(i*(rect_hight_TM+space_rect_tm)),rect_width_TM - 180,rect_hight_TM - 50)
                ...
                for i,tm in enumerate(self.TM_LST):
                    TM_RECT_PARTY_PAGE = pygame.Rect(width - rect_width_TM - 90,42+(i*(rect_hight_TM+space_rect_tm)),rect_width_TM,rect_hight_TM)
...
##                                print('TM deleted')
                    ...
                    if tm.RemovedByUser:
                        ...
                        no_btn_del_tm ,yes_btn_del_tm = draw_multyline_text(my_image,warning_del_tm_text,(0.5*width - 105,0.5*height+15),font,WHITE\
                                                                            ,210,85,(0,5),(30,30,30),['No','Yes'])
            ...
            if methode_lst[self.index_methode] < 4:
                 #drawing
                pygame.draw.polygon(screen,gray,[(cx+X,cy+Y),(cx+X,cy-Y),(cx,cy+V),(cx-X,cy-Y),(cx-X,cy+Y),(cx,cy-V)])
                ...
                pygame.draw.polygon(my_image,COLOR_POLYGONE,[(cx,cy-hp1),(cx+atk1,cy-(atk1*tan30)),(cx+deff1,cy+(deff1*tan30)),(cx,cy+speed1),(cx-spa1,cy+(spa1*tan30)),(cx-spd1,cy-(spd1*tan30))])
                ...
                atk_spa2 = font2_n.render(f'{int(self.spa)}                                                     {int(self.atk)}',True,BLACK)
                ...
                def_spd2 = font2_n.render(f'{int(self.spd)}                                                     {int(self.deff)}',True,BLACK)
                screen.blit(def_spd1,(cx-X-60,cy-Y-30))
                screen.blit(def_spd2,(cx-X-40,cy-Y-10))

            #create description of each methode
            description_rect = pygame.Rect(515,8,states_methode_text.get_width()+17,states_methode_text.get_height() + 6)
    ...
    def HealthBar(self,a,b,z):#,health,max_health,energy,name):
        ...
        ratio = self.health/(self.max_health if self.max_health != 0 else 100) # the max HP reduced to null when self.kill apearntly
...
##        BLACK = (25,25,25)
        ...
        for i in range(len(start_states)):                 
            ...
            if current_states[i] > start_states[i]:
                #print('x')
                pygame.draw.rect(screen, (230,255,230),(x + space,y+z+15,0.22*width,15),0,4)#for boosting########################
                pygame.draw.rect(screen, (0,153,0),(x + space,y+z+15,0.22*width,15),1,4)#for boosting########################
    ...
    def update(self,d):
        ...
        for enemy in enemy_group:
            if pygame.sprite.spritecollide(enemy, bullet_group , False):
                if enemy.alive and enemy.rect.x in range(self.rect.x - 10,self.rect.x + 10) and enemy.stop_draw == False:
                    enemy.health -= 5
...
class Bullet2(pygame.sprite.Sprite):
    def __init__(self,x,y,direction,character):
        ...
        for i in range(1,self.range+1):
            self.image = pygame.transform.scale(self.image,(character.img.get_width()*i,character.img.get_height()*0.5))
...
class Grenade(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.timer <= 0:
            self.kill()
            explosion = Explosion(self.rect.x,self.rect.y - 3*self.rect.size[1],0.5,explosion_anim_direction,0,16,1)
            explosion_group.add(explosion)
            # do damage to anyone that is nearby
            distance_player = (((self.rect.centerx -player.rect.centerx)**2)+((self.rect.centery -player.rect.centery)**2))**0.5
            ...
            for enemy in enemy_group:
                distance_enemy = (((self.rect.centerx -enemy.rect.centerx)**2)+((self.rect.centery -enemy.rect.centery)**2))**0.5
                ...
                if pygame.sprite.spritecollide(enemy, pokeball_group , False):# and not collided_already:
                    if enemy.alive: 
                        ...
                        if CatchingRate(enemy): # function that check catching rate of each wild pokemone.
                            #n = 78
                            player.exp += enemy.lvl * (enemy.max_health + enemy.atk + enemy.deff +enemy.spa + enemy.spd + enemy.energy )// 6
                        ...
                        #if self.timer < 192:
                            #activate animation 
                        cathing_animation = Explosion(self.rect.x,self.rect.y - 3*self.rect.size[1],0.5,into_pokeball_anim_direction,0,72,1,anim_file)
...
##                        self.ground_zero.append(tile_data2)
                        ...
                        player = Player(CHARACTER.name,"player",x*TILE_SIZE,y*TILE_SIZE,CHARACTER.hp,CHARACTER.scale,CHARACTER.speed,CHARACTER.fake_speed,CHARACTER.tm1,CHARACTER.tm2\
                                        ,CHARACTER.typ1,CHARACTER.typ2,CHARACTER.energy,CHARACTER.atk,CHARACTER.deff,CHARACTER.spa,CHARACTER.spd,CHARACTER.tm_lvl)#,25,90)
                        ...
                        for z in os.listdir('images\\game_background1\\pokemon'):
                            if z == str(tile - enemies_num)+'.png':
                                if enemy_collection[tile-enemies_num] in os.listdir('images\\pokemon_images'):
                                    ...
                                    enemy = Player(enemy_collection[tile-enemies_num],'enemy',x*TILE_SIZE,y*TILE_SIZE,pokemon_collection[enemy_collection[tile-enemies_num]].hp,\
                                                   pokemon_collection[enemy_collection[tile-enemies_num]].scale\
                                                   ,pokemon_collection[enemy_collection[tile-enemies_num]].speed,pokemon_collection[enemy_collection[tile-enemies_num]].fake_speed,pokemon_collection[enemy_collection[tile-enemies_num]].tm1,0,\
                                                   pokemon_collection[enemy_collection[tile-enemies_num]].typ1,pokemon_collection[enemy_collection[tile-enemies_num]].typ2,\
                                                   pokemon_collection[enemy_collection[tile-enemies_num]].energy,pokemon_collection[enemy_collection[tile-enemies_num]].atk,\
                                                   pokemon_collection[enemy_collection[tile-enemies_num]].deff,pokemon_collection[enemy_collection[tile-enemies_num]].spa,\
                                                   pokemon_collection[enemy_collection[tile-enemies_num]].spd,pokemon_collection[enemy_collection[tile-enemies_num]].tm_lvl)#,None,None)
                                    enemy_group.add(enemy)

                                    
                  
                                    enemy.lvl =  level*2 + 5 + x//10
                                    enemy.max_health,enemy.atk,enemy.deff,enemy.spa,enemy.spd,enemy.speed,enemy.energy,enemy.reg,enemy.fake_speed  = pokemon_collection[enemy_collection[tile-enemies_num]].states_update(enemy.lvl)
    ...
    def draw_menue(self):
        ...
        for i,t in enumerate(self.tm_lst):
            ...
            if action1_rect.collidepoint(pos):
                pygame.draw.circle(screen,(20,20,20),(action1_rect.centerx-action1_rect.h//2,action1_rect.centery),action1_rect.h//2,3)
        ...
        #scroll list
        if down_rect_trainer.collidepoint(pos) and y_axis_trainer > -z - (fixed_y*6) + 550:#window_rect.bottom:#and y_axis_trainer > -z+(space*window_rect.h/(y0-space)):# and y_axis_trainer > 0:
        ...
        pygame.draw.polygon(screen,(100,72,2),((down_rect_trainer.x+5,down_rect_trainer.top+5),(down_rect_trainer.centerx,down_rect_trainer.bottom-5),(down_rect_trainer.right-5,down_rect_trainer.top+5)),3)
        pygame.draw.polygon(screen,(100,72,2),((up_rect_trainer.x+5,up_rect_trainer.bottom-5),(up_rect_trainer.centerx,up_rect_trainer.top+5),(up_rect_trainer.right-5,up_rect_trainer.bottom-5)),3)
        ...
        if self.warning:
            ...
            answer_no_rect,answer_yes_rect = draw_multyline_text(my_image,text_tm_del,(1.5*x_pos,1.5*y_pos),font,(60,60,60),200,250,\
                                (x_pos,y_pos),(150,150,150,150) , text2 = ['No','Yes'])
...
settings = [screen_width,screen_height,fps,sound_setting,music_setting,font_size]#,font_color_r,font_color_g,font_color_b] 
...
while run:       
    ...
    if start_game == False:
        ...
        if fullscreen_button.draw(screen):
            ...
            oak_img = pygame.transform.scale(oak_img,(SCREEN_WIDTH - choose_starter_img.get_width(),1.2*SCREEN_HEIGHT-10))
...
##            player = party_lst[trainer.switch_pokemon]
        ...
        draw_text(f'Game Level: {level}',font,set_color,50,pygame.display.get_surface().get_size()[1] - 2*TILE_SIZE)
    ...
    ##                    screen = pygame.display.set_mode((screen_width.value,screen_height.value))
                    ...
                    defult_rect_btn,full_screen_rect_btn,control_setting_rect_btn = SETTINGS.Draw_special_settings(screen,setting_window)
                    ...
                    #define close window rect (as on regullar pc)
                        ...
                        for func in settings:
                            ...
                            if func.rect.collidepoint((pos)) and pygame.mouse.get_pressed()[0] and pos[0] in range(func.rect_line.x,func.rect_line.x+func.line_width + 1):
                            ...
                            elif func.rect_line_transperent.collidepoint((pos)) and pygame.mouse.get_pressed()[0]: #clicking area are wider then visible line
                                func.rect.centerx = pos[0]
                                func.update(pos[0])
                        ...
                        if full_screen_rect_btn.collidepoint((pos)):
                            ...
                            if pygame.mouse.get_pressed()[0]:
                                screen = pygame.display.set_mode((0,0))
                                pokemon_party_btn = button.Button(pygame.display.get_surface().get_size()[0]-40,20,pokemon_party_img,0.3,0.3)
                                #controller_setting_bg_image = pygame.transform.scale(controller_setting_bg_image,(pygame.display.get_surface().get_size()))
                    ...
                    if setting_controller:
                        ...
                        controller_window = pygame.Surface((setting_controller_width*0.85,setting_controller_height*0.9))
                        ...
                        closing_rect_x_rect = rect_in_sub_window.move(controller_window_rect.right+x_cord_control_window,y_cord_control_window)
...
##                        #define close window rect (as on regullar pc) # i choode to define those rect for learning the rect.move 
                        ...
                        controller_setting_bg_image = pygame.transform.scale(controller_setting_bg_image,(controller_window.get_size()[0],controller_window.get_size()[1]-20))
                        ...
                        if SETTINGS.IS_DEFAULT(controller_window,x_cord_control_window,y_cord_control_window): # this func take the x,y_fix position due to collideing with rect issue
            ...
            for enemy in enemy_group:
                ...
                if enemy.stop_draw == False:
                    ...
                    if enemy.rect.x in range(player.rect.x -x_dis,player.rect.x +x_dis) and enemy.rect.y in range(player.rect.y -y_dis,player.rect.y +y_dis)\
            ...
            if player.alive:
                ...
                if player.exp > lvl_up_pts(player.lvl):
                        ...
                        player.max_health,player.atk,player.deff,player.spa,player.spd,player.speed,player.energy,player.reg,player.fake_speed = CHARACTER.states_update(player.lvl)
                        ...
                        print(player.max_health,player.atk,player.deff,player.spa,player.spd,player.speed,player.energy,player.reg,player.fake_speed)
...
##                        print('Spd(player):',player.spd,'Spd(char):',CHARACTER.spd,'start_spd:',player.start_spd)
                ...
                if player.speed > player.start_speed:
                    ...
                    speed_img = pygame.transform.scale(speed_img,(int(player.rect.size[0]*1.5),int(player.rect.size[1]*1.5)))
